...
export default function ProgrammingToolsPage() {
  ...
  const filteredTools = programmingTools.filter((tool) => {
    const matchesSearch =
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 text-lg text-muted-foreground">
              探索GitHub Copilot、Cursor、v0等AI编程工具，
              了解如何使用AI提升编程效率，无论你是新手还是有经验的开发者。
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          {filteredTools.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-muted-foreground">没有找到匹配的编程工具</p>
              <Button
                variant="ghost"
                className="mt-4"
                onClick={() => {
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredTools.map((tool) => (
                <Link key={tool.id} href={`/programming-tools/${tool.id}`}>
